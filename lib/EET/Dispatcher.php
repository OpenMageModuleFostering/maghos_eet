...
class EET_Dispatcher {
    ...
    private function prepareData(EET_Receipt $receipt, $check = false) {
        ...
        $tax0 = $receipt->getTax(EET_Receipt::TAX_ZERO);
        $tax1 = $receipt->getTax(EET_Receipt::TAX_BASE);
        $tax2 = $receipt->getTax(EET_Receipt::TAX_LOWER1);
        $tax3 = $receipt->getTax(EET_Receipt::TAX_LOWER2);
        
        $tax1Used = $receipt->getTax(EET_Receipt::TAX_BASE, true);
        $tax2Used = $receipt->getTax(EET_Receipt::TAX_LOWER1, true);
        $tax3Used = $receipt->getTax(EET_Receipt::TAX_LOWER2, true);
		...
		$body = [
			...
			'zakl_nepodl_dph' => $this->formatPrice($tax0[0]),
			'zakl_dan1' => $this->formatPrice($tax1[0]),
			'dan1' => $this->formatPrice($tax1[1]),
			'zakl_dan2' => $this->formatPrice($tax2[0]),
			'dan2' => $this->formatPrice($tax2[1]),
			'zakl_dan3' => $this->formatPrice($tax3[0]),
			'dan3' => $this->formatPrice($tax3[1]),
			'cest_sluz' => $this->formatPrice($receipt->service_total),
			'pouzit_zboz1' => $this->formatPrice($tax1Used[0]+$tax1Used[1]),
			'pouzit_zboz2' => $this->formatPrice($tax2Used[0]+$tax2Used[1]),
			'pouzit_zboz3' => $this->formatPrice($tax3Used[0]+$tax3Used[1]),
